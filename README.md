# 🌸 AnimeHay Enhanced Helpers 🌸

<img src="assets/banner.png" width="50%" alt="AnimeHay Suite">

A super cute and handy script suite to make watching on AnimeHay an absolute breeze! ✨

### 🎀 What's Inside?
- ☁️ **[Animehay2Anilist](docs/README_Animehay2Anilist.md)**: Auto-sync your progress right to your AniList!
- 💖 **[Episode Tracker](docs/README_animeHay_epTracker.md)**: Always know which episode is next.
- ⏩ **[Smart Skipper](docs/README_animehaySmartSkipper.md)**: Zip past intros and outros automatically.
- ⭐ **[Gold Score Highlight](docs/README_animeHay_goldScore_Highlight.md)**: Make top-tier 9.0+ anime sparkle!
- 🎵 **[Volume Memory](docs/README_animeHay_volumeMemory.md)**: Keeps your perfect volume saved.
- 📺 **[Fullscreen Controller](docs/README_animeHay_forceFS_ifNext.md)**: Hop into fullscreen for the next episode.
- 🎧 **[EchoSkip – Audio OST Skipper](docs/README_animeHay_echoSkipper.md)**: Teach it your OP/ED once, it skips forever via audio fingerprinting!

Happy watching! 🍿✨
